...
import 'package:sixvalley_vendor_app/data/repository/shop_repo.dart';
...
class SellerProvider extends ChangeNotifier {
  ...
  Future addProductImage(XFile pickedLogo,String type, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse response = await shopRepo.addImage( pickedLogo, type);
    if(response.response != null && response.response.statusCode == 200) {
      _isLoading = false;
  ...
  Future<void> addProduct(BuildContext context, Product product, AddProductModel addProduct, List<String> productImages, String thumbnail, String metaImage, String token, bool isAdd,bool isActiveColor) async {
    ...
    ApiResponse response = await shopRepo.addProduct(product,addProduct ,_fields, productImages, thumbnail, metaImage, token, isAdd, isActiveColor);
    if(response.response != null && response.response.statusCode == 200) {
      _productCode.clear();
