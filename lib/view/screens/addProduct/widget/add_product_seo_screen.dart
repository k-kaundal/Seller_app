    ...
    if (isRoute) {
      if(_update){
        if(thumbnailImage=='' && metaImage ==''){
          total = Provider.of<SellerProvider>(context,listen: false).productImage.length;
      ...
      if(metaImage ==''){
        total = Provider.of<SellerProvider>(context,listen: false).productImage.length + 1;
      }else{

        total = Provider.of<SellerProvider>(context,listen: false).productImage.length + 2;
      }
      if(counter == total) {
        counter++;
        Provider.of<SellerProvider>(context,listen: false).addProduct(
            context, _product, _addProduct, productImage, thumbnailImage, metaImage,
            Provider.of<AuthProvider>(context,listen: false).getUserToken(),
            !_update,Provider.of<SellerProvider>(context,listen: false).attributeList[0].active);
      }
    } else {
        ...
        child: Padding(
          ...
          child: Consumer<SellerProvider>(
            builder: (context, resProvider, child){
              print('=============baby=========>${resProvider.digitalProductFileName}');
