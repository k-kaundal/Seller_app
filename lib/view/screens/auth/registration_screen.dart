...
import 'package:sixvalley_vendor_app/view/screens/auth/widgets/successfully_registered_screen.dart';
...
class _RegistrationScreenState extends State<RegistrationScreen> with TickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(title: getTranslated('shop_application', context), isBackButtonExist: false),
            ...
            return Container(height: 70,
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: CustomButton(btnTxt: getTranslated('submit', context), onTap: !authProvider.isTermsAndCondition? null: (){
                      if(authProvider.firstNameController.text.trim().isEmpty){
                        showCustomSnackBar(getTranslated('first_name_is_required', context), context);
                      }else if(authProvider.lastNameController.text.trim().isEmpty){
                        showCustomSnackBar(getTranslated('last_name_is_required', context), context);
                        ...
                        authProvider.registration(context, registerModel).then((value){
                          if(value.response.statusCode == 200){
                            Navigator.of(context).pushReplacement(MaterialPageRoute(
                                builder: (BuildContext context) => SuccessfullyRegisteredScreen()));
                          }
                        });
                      }
